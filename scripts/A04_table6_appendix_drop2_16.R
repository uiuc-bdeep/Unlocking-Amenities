...
TD_matched<-TD_matched %>% filter(bin2!=1)
...
texreg(list(reg1,
       ...
       ,file = "views/table_6_drop216.tex")
          


round(summary(iv1$stage1, lhs="homicide.risk",digits=3)$coefficients,3)
round(summary(iv1$stage1, lhs="homicide.risk")$iv1fstat,3)

iv3_fs<- felm(delta.price ~    1 + median_age_block + median_income_block
           + pop.density_block + white.p_block + black.p_block + hispanic.p_block
           + vacant.p_block + renter.p_block
           |time1 + time0 + park_city | (delta.hom.risk  ~ delta.iv.hom.risk   ) | tract_city,
           data=TD_rep_sale)

round(summary(iv3_fs$stage1, lhs="delta.hom.risk",digits=3)$coefficients,3)
round(summary(iv3_fs$stage1, lhs="delta.hom.risk")$iv1fstat,3)
...
round(summary(match_iv4_fs$stage1, lhs="homicide.risk",digits=3)$coefficients,3)
round(summary(match_iv4_fs$stage1, lhs="homicide.risk")$iv1fstat,3)
